...
def model_wrapper(
    ...
    def noise_pred_fn(input_ids_x,x, t_continuous, cond=None):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(input_ids_x, x, t_input, **model_kwargs)
        else:
    ...
    def model_fn(input_ids_x,x, t_continuous):
        """
        The noise predicition model function that is used for DPM-Solver.
        """
        if guidance_type == "uncond":
            return noise_pred_fn(input_ids_x,x, t_continuous)
        elif guidance_type == "classifier":
            assert classifier_fn is not None
            t_input = get_model_input_time(t_continuous)
            cond_grad = cond_grad_fn(x, t_input)
            sigma_t = noise_schedule.marginal_std(t_continuous)
            noise = noise_pred_fn(input_ids_x,x, t_continuous)
            return noise - guidance_scale * sigma_t * cond_grad
        elif guidance_type == "classifier-free":
            if guidance_scale == 1. or unconditional_condition is None:
                return noise_pred_fn(input_ids_x,x, t_continuous, cond=condition)
            else:
                x_in = torch.cat([x] * 2)
...
class DPM_Solver:
    def __init__(
        ...
        """
        self.model = lambda input_ids_x ,x, t: model_fn(input_ids_x,x, t.expand((x.shape[0])))
    ...
    def noise_prediction_fn(self, input_ids_x,x, t):
        """
        Return the noise prediction model.
        """
        return self.model(input_ids_x,x, t)

    def data_prediction_fn(self, input_ids_x, x, t):
        """
        Return the data prediction model (with corrector).
        """
        noise = self.noise_prediction_fn(input_ids_x, x, t)
    ...
    def model_fn(self, input_ids_x,x, t):
        """
        Convert the model to the noise prediction model or the data prediction model. 
        """
        if self.algorithm_type == "dpmsolver++":
            return self.data_prediction_fn(input_ids_x,x, t)
        else:
    ...
    def sample(self, input_ids_x ,x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        method='multistep', lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver',
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                x = self.model_fn(input_ids_x,x, t)
