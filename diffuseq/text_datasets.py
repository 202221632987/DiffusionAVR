...
def load_data_text(
    ...
    if "pretrain" in data_args.notes:
        ...
        training_data = get_corpus_pretrain(data_args, seq_len, split=split, loaded_vocab=loaded_vocab,
                                            split_num=data_args.data_split_num)
    else:
    ...
    def merge_and_mask(group_lst):
        ...
        for i in range(len(group_lst['input_id_x'])):
            ...
            if len(src) + len(trg) > seq_len - 3:
                ...
                while len(src) + len(trg) < seq_len - 3:
                    src = src + [vocab_dict.pad_token_id]
            src.append(end_token)
            trg.append(end_token)
            lst.append(src + [vocab_dict.sep_token_id] + trg)
            if len(src + [vocab_dict.sep_token_id] + trg) != 768:
                print("error")
...
def get_corpus(data_args, seq_len, split='train', loaded_vocab=None):
    ...
    if split == 'train':
        ...
        path = f'{data_args.data_dir}'
        train_data_whole = datasets.load_dataset(path, split="train")
        ...
        for index, content in train_data.iterrows():
            sentence_lst['src'].append(content['source'].strip())
            sentence_lst['trg'].append(content['target'].strip())
    elif split == "valid":
        ...
        df = pd.DataFrame({"source": train_data_whole["source"], "target": train_data_whole["target"]})
        train_data, val_data = train_test_split(df, test_size=0.1238, random_state=42)
        for index, content in val_data.iterrows():
            sentence_lst['src'].append(content['source'].strip())
            sentence_lst['trg'].append(content['target'].strip())
    elif split == 'test':
        ...
        path = f'{data_args.data_dir}'
        train_data_whole = datasets.load_dataset(path, split="test")
        df = pd.DataFrame({"source": train_data_whole["source"], "target": train_data_whole["target"]})
