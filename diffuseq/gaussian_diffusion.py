...
class GaussianDiffusion:
    ...
    def training_losses_seq2seq(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        target = x_start
        model_output = model(input_ids_x,x_t, self._scale_timesteps(t), **model_kwargs)
        ...
        terms["loss"] = terms["mse"] + decoder_nll + tT_loss + terms["nll"]
...
class _WrappedModel:
    ...
    def __call__(self,input_ids, x, ts, **kwargs):
        map_tensor = th.tensor(self.timestep_map, device=ts.device, dtype=ts.dtype)
        new_ts = map_tensor[ts]
        if self.rescale_timesteps:
            new_ts = new_ts.float() * (1000.0 / self.original_num_steps)
        return self.model(input_ids,x,new_ts, **kwargs)
