...
def clean_tokens(tokens):
    ...
    tokens = tokens.replace("<S2SV_StartBug>", " <S2SV_StartBug> ")
    tokens = tokens.replace("<S2SV_EndBug>", " <S2SV_EndBug> ")
    tokens = tokens.replace("<S2SV_ModStart>", " <S2SV_ModStart> ")
    tokens = tokens.replace("<S2SV_ModEnd>", " <S2SV_ModEnd> ")
    if tokens.startswith(" <S2SV_ModStart> ") :
        tokens = tokens[1:]
    tokens = tokens.strip("\n")
    tokens = tokens.strip()
    return tokens
...
def main():
    ...
    for cond in tqdm(all_test_data):
        ...
        for seq, input_mask in zip(input_ids_x, input_ids_mask_ori):
            ...
            word_lst_ref.append(clean_tokens(tokenizer.decode_token(seq[len_x:])))

        for recover,ref in zip(word_lst_recover,word_lst_ref):
            if recover == ref:
                accuracy += 1
            num += 1
        fout = open(out_path, 'a')
