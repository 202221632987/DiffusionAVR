...
class myTokenizer():
    ...
    def __init__(self, args):
        ...
        elif args.vocab == 'codebert-base':
            tokenizer = RobertaTokenizer.from_pretrained(args.vocab)

            tokenizer.add_tokens(
                ["<S2SV_StartBug>", "<S2SV_EndBug>", "<S2SV_blank>", "<S2SV_ModStart>", "<S2SV_ModEnd>"])
            self.sep_token_id = tokenizer.sep_token_id
            self.pad_token_id = tokenizer.pad_token_id
            self.tokenizer = tokenizer
            # save
            tokenizer.save_pretrained(args.checkpoint_path)
        elif args.vocab == 'codet5':
    ...
    def encode_token(self, sentences):
        ...
        for i in range(len(sentences)):
            input_ids.append(self.tokenizer.encode(sentences[i], return_tensors='pt').tolist()[0])
